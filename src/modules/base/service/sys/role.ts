...
export class BaseSysRoleService extends BaseService {
  ...
  async modifyBefore(data: any, type: 'delete' | 'update' | 'add'): Promise<void> {
      if (type === 'update') {
        const role = await this.baseSysRoleEntity.findOneBy({ id: data.id });
        if (role.tenantId !== this.ctx.admin.tenantId) {
