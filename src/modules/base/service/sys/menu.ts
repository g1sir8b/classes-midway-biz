...
export class BaseSysMenuService extends BaseService {
  ...
  async list() {
    const menus = await this.getMenus(
      ...
      this.ctx.admin.tenantId,
      this.ctx.admin.username === 'admin' || await this.baseSysPermsService.isAdmin(this.ctx.admin.roleIds)
    );
    if (!_.isEmpty(menus)) {
      menus.forEach((e: any) => {
   ...
   * @param {[]} roleIds 数组
   */
  async getPerms(roleIds, tenantId) {
    let perms = [];
      ...
      if (tenantId) {
        find.andWhere(qb => {
          const subQuery = qb
            .subQuery()
            .select('mm.menuId')
            .from(BaseSysModuleMenuEntity, 'mm')
            .innerJoin(
              BaseSysTenantModuleEntity,
              'tm',
              'tm.moduleId = mm.moduleId'
            )
            .where('tm.tenantPrimaryId = :tenantId', { tenantId });
   ...
   * @param isAdmin 是否是超管
   */
  async getMenus(roleIds, tenantId, isAdmin) {
    const find = this.baseSysMenuEntity.createQueryBuilder('a');
    if (!isAdmin) {
      find.innerJoinAndSelect(
        BaseSysRoleMenuEntity,
