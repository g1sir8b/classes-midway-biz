...
export class BaseSysDepartmentService extends BaseService {
  ...
  async add(param: any | any[]): Promise<Object> {
    ...
    // 更新部门权限
    await this.baseSysPermsService.refreshPerms(this.ctx.admin.userId);
  ...
  async list() {
    ...
    if (this.ctx.admin.username !== 'admin' || !(this.ctx.admin.tenantId && this.baseSysPermsService.isAdmin(this.ctx.admin.roleIds)))
      find.andWhere('a.id in (:...ids)', {
        ids: !_.isEmpty(permsDepartmentArr) ? permsDepartmentArr : [null],
      });
    if (this.ctx.admin.tenantId) {
      find.andWhere('a.tenantId = :tenantId', { tenantId: this.ctx.admin.tenantId });
    }
   ...
   * @param isAdmin 是否超管
   */
  async getByRoleIds(roleIds: number[], isAdmin, tenantId) {
