...
export class TenantSubscriber implements EntitySubscriberInterface<any> {
   ...
   */
  getTenantId(): number | undefined | boolean {
    let ctx, url, tenantId;
    ctx = this.getCtx();
    if (!ctx || !this.checkHandler()) return false;
    url = ctx?.url;
    // 忽略用户
  ...
  afterSelectQueryBuilder(queryBuilder: SelectQueryBuilder<any>) {
    ...
    // if (queryBuilder?.expressionMap?.aliases?.some(alias => alias.tablePath === 'base_sys_tenant')) {
    //   return;
    // }

    if (tenantId === undefined) {
      queryBuilder.andWhere(
        `${queryBuilder.alias ? queryBuilder.alias + '.' : ''}tenantId is null`
      );
    }
